...
/*      For Mixxx version 2.1.0-alpha-pre                       */
/****************************************************************/
...
TraktorS2MK2.registerOutputPackets = function() {
  ...
  Output1.addOutput("[Channel1]", "PeakIndicator", 0x17, "B");
  Output1.addOutput("[Channel2]", "PeakIndicator", 0x0F, "B");
  ...
  if(PlayCueDark) {
    TraktorS2MK2.linkDeckOutputs("cue_indicator", TraktorS2MK2.outputCallbackDark);
    TraktorS2MK2.linkDeckOutputs("play_indicator", TraktorS2MK2.outputCallbackDark);
  } else {
  ...
  TraktorS2MK2.linkDeckOutputs("hotcue_1_enabled", TraktorS2MK2.outputCueCallback);
  TraktorS2MK2.linkDeckOutputs("hotcue_2_enabled", TraktorS2MK2.outputCueCallback);
  TraktorS2MK2.linkDeckOutputs("hotcue_3_enabled", TraktorS2MK2.outputCueCallback);
  ...
  engine.connectControl("[Channel2]", "VuMeter", "TraktorS2MK2.onVuMeterChanged");

  engine.connectControl("[Channel1]", "loop_enabled", "TraktorS2MK2.onLoopEnabledChanged");
  engine.connectControl("[Channel2]", "loop_enabled", "TraktorS2MK2.onLoopEnabledChanged");
}
...
TraktorS2MK2.pointlessLightShow = function() {
...
TraktorS2MK2.init = function(id) {
  //TraktorS2MK2.pointlessLightShow()
  ...
  //TraktorS2MK2.lightDeck("[Preview[Channel1]]");
  //TraktorS2MK2.lightDeck("[Channel1]");
  //TraktorS2MK2.lightDeck("[Channel2]");

  HIDDebug("TraktorS2MK2: done init");
}

TraktorS2MK2.debugLights = function() {
...
TraktorS2MK2.incomingData = function(data, length) {
  // There are no partial packets in S2 so we can handle them right away
  ...
  if (length === 15) {
    if (this.partial_packet.length !== 64) {
      HIDDebug("Traktor S2MK2: Received second half of message but don't have first half, ignoring");
      return;
    }
    ...
    TraktorS2MK2.controller.parsePacket(this.partial_packet, partial_length + length);
    // Clear out the partial packet
    this.partial_packet = Object();
    return;
  }

  HIDDebug("Traktor S2MK2: Unhandled packet size: " + length);
}
...
TraktorS2MK2.shiftHandler = function(field) {
  var group = field.id.split(".")[0];
  TraktorS2MK2.controller.shift_pressed[group] = field.value;
  TraktorS2MK2.outputCallback(field.value, field.group, "!shift");
}

TraktorS2MK2.loadTrackHandler = function(field) {
  var splitted = field.id.split(".");
  var group = splitted[0];
  if (TraktorS2MK2.controller.shift_pressed[group]) {
...
TraktorS2MK2.effectEnabledHandler = function(field) {
  var splitted = field.id.split(".");
  var group = splitted[0];
  if (field.value) {
...
TraktorS2MK2.syncEnabledHandler = function(field) {
  var now = Date.now();
  ...
  if (TraktorS2MK2.controller.shift_pressed[group]) {
    ...
    if (field.value === 1) {
      TraktorS2MK2.controller.sync_enabled_time[field.group] = now;
      ...
      if (now - TraktorS2MK2.controller.sync_enabled_time[field.group] > 300) {
        engine.setValue(field.group, "sync_enabled", 1);
        return;
      }
      engine.setValue(field.group, "sync_enabled", 0);
...
TraktorS2MK2.loopActivateHandler = function(field) {
  var splitted = field.id.split(".");
  var group = splitted[0];
  if (TraktorS2MK2.controller.shift_pressed[group]) {
...
TraktorS2MK2.cueHandler = function(field) {
  var splitted = field.id.split(".");
  var group = splitted[0];
  if (TraktorS2MK2.controller.shift_pressed[group]) {
    if (TraktorS2MK2.ShiftCueButtonAction == "REWIND") {
      if (field.value === 0) {
        return;
      }
      engine.setValue(field.group, "start_stop", 1);
    } else if (TraktorS2MK2.ShiftCueButtonAction == "REVERSEROLL") {
...
TraktorS2MK2.playHandler = function(field) {
  if (field.value === 0) {
    return;
  }
  var splitted = field.id.split(".");
...
TraktorS2MK2.previewDeckHandler = function(field) {
  if (field.value === 0) {
    return;
  }
...
TraktorS2MK2.jogTouchHandler = function(field) {
  if (TraktorS2MK2.controller.wheelTouchInertiaTimer[field.group] != 0) {
    // The wheel was touched again, reset the timer.
    engine.stopTimer(TraktorS2MK2.controller.wheelTouchInertiaTimer[field.group]);
    TraktorS2MK2.controller.wheelTouchInertiaTimer[field.group] = 0;
  }
  if (field.value !== 0) {
    ...
    if (inertiaTime < 100) {
      ...
      TraktorS2MK2.controller.wheelTouchInertiaTimer[field.group] = engine.beginTimer(
          inertiaTime, "TraktorS2MK2.finishJogTouch(\"" + field.group + "\")", true);
    }
  }
}

TraktorS2MK2.finishJogTouch = function(group) {
  TraktorS2MK2.controller.wheelTouchInertiaTimer[group] = 0;
  var deckNumber = TraktorS2MK2.controller.resolveDeck(group);
  // No vinyl button (yet)
  /*if (this.vinylActive) {
  ...
  if (play != 0) {
    ...
    if (scratchRate < 0.01) {
      ...
      // Check again soon.
      TraktorS2MK2.controller.wheelTouchInertiaTimer[group] = engine.beginTimer(
              100, "TraktorS2MK2.finishJogTouch(\"" + group + "\")", true);
    }
  }
}

TraktorS2MK2.jogMoveHandler = function(field) {
...
TraktorS2MK2.wheelDeltas = function(group, value) {
  ...
  if (group[8] === "1" || group[8] === "3") {
    prev_tick = TraktorS2MK2.controller.last_tick_val[0];
    prev_time = TraktorS2MK2.controller.last_tick_time[0];
    TraktorS2MK2.controller.last_tick_val[0] = tickval;
    TraktorS2MK2.controller.last_tick_time[0] = timeval;
  } else {
    prev_tick = TraktorS2MK2.controller.last_tick_val[1];
    prev_time = TraktorS2MK2.controller.last_tick_time[1];
    TraktorS2MK2.controller.last_tick_val[1] = tickval;
    TraktorS2MK2.controller.last_tick_time[1] = timeval;
  }
...
TraktorS2MK2.hotcueHandler = function(field) {
  var group = field.id.split(".")[0];
...
TraktorS2MK2.quantizeHandler = function(field) {
  if (field.value === 0) {
    return;
  }
  TraktorS2MK2.master_quantize = !TraktorS2MK2.master_quantize;
  engine.setValue("[Channel1]", "quantize", TraktorS2MK2.master_quantize);
  engine.setValue("[Channel2]", "quantize", TraktorS2MK2.master_quantize);
  TraktorS2MK2.controller.setOutput("[Master]", "!quantize", 0x7F * TraktorS2MK2.master_quantize, true);
}

TraktorS2MK2.callbackPregain = function(field) {
  // TODO: common-hid-packet-parser looks like it should do deltas, but I can't get them to work.
  prev_pregain = TraktorS2MK2.controller.prev_pregain[field.group];
  TraktorS2MK2.controller.prev_pregain[field.group] = field.value;
  var delta = 0;
...
TraktorS2MK2.callbackLoopMove = function(field) {
  // TODO: common-hid-packet-parser looks like it should do deltas, but I can't get them to work.
  var splitted = field.id.split(".");
  var group = splitted[0]
  prev_loopmove = TraktorS2MK2.controller.prev_loopmove[group];
  TraktorS2MK2.controller.prev_loopmove[group] = field.value;
  var delta = 0;
...
TraktorS2MK2.callbackLoopSize = function(field) {
  var splitted = field.id.split(".");
  var group = splitted[0]
  prev_loopsize = TraktorS2MK2.controller.prev_loopsize[group];
  TraktorS2MK2.controller.prev_loopsize[group] = field.value;
  var delta = 0;
...
TraktorS2MK2.callbackBrowse = function(field) {
  // TODO: common-hid-packet-parser looks like it should do deltas, but I can't get them to work.
  prev_browse = TraktorS2MK2.controller.prev_browse;
  TraktorS2MK2.controller.prev_browse = field.value;
  var delta = 0;
...
TraktorS2MK2.outputChannelCallback = function(value,group,key) {
  ...
  TraktorS2MK2.controller.setOutput(group, key, led_value, !TraktorS2MK2.controller.freeze_lights);
}
...
TraktorS2MK2.outputCueCallback = function(value, group, key) {
  var RGB_value = [0, 0, 0];
  ...
  TraktorS2MK2.controller.setOutput(group, key, RGB_value[0], false);
  TraktorS2MK2.controller.setOutput(group, "!" + key + "_G", RGB_value[1], false);
  TraktorS2MK2.controller.setOutput(group, "!" + key + "_B", RGB_value[2], !TraktorS2MK2.controller.freeze_lights);
}
...
TraktorS2MK2.onVuMeterChanged = function(value, group, key) {
  ...
  TraktorS2MK2.controller.OutputPackets["output1"].send();
}
